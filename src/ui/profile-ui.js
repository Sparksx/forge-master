...
export function renderProfileContent(user, onLogout) {
    ...
    if (currentUser) {
        ...
        editBtn.title = `Change username (\uD83D\uDC8E ${formatNumber(USERNAME_CHANGE_COST)})`;
...
function showUsernameChangeUI(container, currentUser) {
    ...
    if (getDiamonds() < USERNAME_CHANGE_COST) {
        ...
        const error = createElement('div', 'profile-rename-error-msg', `Not enough diamonds! Need \uD83D\uDC8E ${formatNumber(USERNAME_CHANGE_COST)}`);
...
function renderSettingsContent() {
    ...
    if (currentUser) {
        ...
        linkSection.appendChild(createElement('div', 'settings-section-title', 'Linked Accounts'));
